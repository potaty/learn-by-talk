...
*/
    ...
    window.JSON && window.JSON.stringify || function() {
        ...
        function b(c, q) {
            ...
            case "object":
                ...
                if (m && "object" === typeof m) {
                    ...
                    for (t = 0; t < j; t += 1)
                        r = m[t],
                        "string" === typeof r && (g = b(r, e)) && h.push(d(r) + (f ? ": " : ":") + g)
                } else
                    for (r in e)
                        Object.hasOwnProperty.call(e, r) && (g = b(r, e)) && h.push(d(r) + (f ? ": " : ":") + g);
                g = 0 === h.length ? "{}" : f ? "{\n" + f + h.join(",\n" + f) + "\n" + l + "}" : "{" + h.join(",") + "}";
                f = l;
                return g
            }
        ...
        var c = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, f, p, q = {
            "\b": "\\b",
            "\t": "\\t",
            "\n": "\\n",
            "\f": "\\f",
    ...
    function qa(a, d) {
        //console.log(a);
        if (a[0].indexOf("http://") == 0) a[0] = "https://" + a[0].substr(7);
        //console.log(a);
        var b = a.join(ia)
          , c = [];
        if (!d)
    ...
    function ra(a, d) {
        function b() {
            f + d > G() ? (clearTimeout(c),
            c = setTimeout(b, d)) : (f = G(),
            a())
        }
        var c, f = 0;
        return b
    ...
    function N(a, d) {
        if (a && d)
            if (a && (Array.isArray && Array.isArray(a) || "number" === typeof a.length))
                ...
                for (b in a)
                    a.hasOwnProperty && a.hasOwnProperty(b) && d.call(a[b], b, a[b])
    }
    function ua(a, d) {
        var b = [];
        N(a || [], function(a, f) {
    ...
    function Aa(a, d) {
        ...
        f = c[0];
        for (q = 1; q < b; q++)
            c[q] = GibberishAES.l.m(c[q - 1].concat(p)),
            f = f.concat(c[q]);
        c = f.slice(0, 4 * U);
    ...
    function Fa(a, d, b, c) {
        ...
        for (q = 0; q < f; q++)
            p.push(a.slice(16 * q, 16 * (q + 1)));
        for (q = p.length - 1; 0 <= q; q--)
            m[q] = Ga(p[q], d),
            m[q] = 0 === q ? Da(m[q], b) : Da(m[q], p[q - 1]);
        for (q = 0; q < f - 1; q++)
            u += ya(m[q]);
        var u = u + ya(m[q], y), x;
        if (c)
    ...
    function Na(a) {
        var d = [], b = xa ? [0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3] : [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11], c;
        for (c = 0; 16 > c; c++)
            d[c] = a[b[c]];
        return d
    }
    ...
    function Ca(a) {
        var d = [], b = [], c, f, p = [];
        for (c = 0; c < U; c++)
            f = [a[4 * c], a[4 * c + 1], a[4 * c + 2], a[4 * c + 3]],
            d[c] = f;
        for (c = U; c < 4 * (T + 1); c++) {
            d[c] = [];
            for (a = 0; 4 > a; a++)
        ...
        for (c = 0; c < T + 1; c++) {
            p[c] = [];
            for (b = 0; 4 > b; b++)
                p[c].push(d[4 * c + b][0], d[4 * c + b][1], d[4 * c + b][2], d[4 * c + b][3])
        }
        return p
    }
    function cb(a) {
    ...
    function eb(a, d) {
        ...
        for (i = 0; i < a.length; i += d)
            b[i / d] = parseInt(a.substr(i, d), 16);
        return b
    }
    function fb(a) {
        for (var d = [], b = 0; 256 > b; b++) {
            for (var c = a, f = b, p = v, q = v, p = q = 0; 8 > p; p++)
                q = 1 == (f & 1) ? q ^ c : q,
                c = 127 < c ? 283 ^ c << 1 : c << 1,
                f >>>= 1;
            d[b] = q
    ...
    for (i = 0; i < gb.length; i++)
        hb[gb[i]] = i;
    Pa = hb;
    var db = eb("01020408102040801b366cd8ab4d9a2f5ebc63c697356ad4b37dfaefc591", 2), ab = fb(2), bb = fb(3), $a = fb(9), Ua = fb(11), Za = fb(13), Ta = fb(14), ib, jb = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", kb = jb.split("");
    "function" === typeof Array.indexOf && (jb = kb);
    ib = {
        encode: function(a) {
            var d = [], b = "", c;
            for (c = 0; c < 16 * a.length; c++)
                d.push(a[Math.floor(c / 16)][c % 16]);
            for (c = 0; c < d.length; c += 3)
                b += kb[d[c] >> 2],
                b += kb[(d[c] & 3) << 4 | d[c + 1] >> 4],
                b = d[c + 1] !== v ? b + kb[(d[c + 1] & 15) << 2 | d[c + 2] >> 6] : b + "=",
                b = d[c + 2] !== v ? b + kb[d[c + 2] & 63] : b + "=";
            a = b.slice(0, 64);
            for (c = 1; c < Math.ceil(b.length / 64); c++)
                a += b.slice(64 * c, 64 * c + 64) + (Math.ceil(b.length / 64) == c + 1 ? "" : "\n");
            return a
        },
        decode: function(a) {
            var a = a.replace(/\n/g, ""), d = [], b = [], c = [], f;
    ...
    O = {
        ...
        Hash: {
            MD5: function(a) {
                ...
                var m = [], u, x, t, r, g, j, h, l, e = eb("", 8), m = a.length;
                u = m + 8;
                x = 16 * ((u - u % 64) / 64 + 1);
                t = [];
                for (g = r = 0; g < m; )
    ...
    if (!window.PUBNUB) {
        var lb = function(a, d) {
          ...
          , V = function(a, d, b) {
            if (b)
                ...
                return a && a.getAttribute && a.getAttribute(d)
        }
          , sb = function(a, d) {
            for (var b in d)
        ...
        var Gb = {
          ...
          , Jb = function(a) {
            ...
            function r() {
                clearTimeout(W);
                !J || 500 <= J || 1 > J || !va(B, y).length ? Wa = A : (Wa = y,
            ...
            function j(a, b, c) {
                ...
                return d ? (c && c("Presence Heartbeat value invalid. Valid range ( x > 5 or x = 0). Current Value : " + (b || 5)),
                b || 5) : a
            }
            function h(a) {
                var b = ""
            ...
            function aa(a) {
                ...
                for (var a = "", f = 0; f < d.length; f++) {
                    ...
                    128 > e ? a += String.fromCharCode(e) : (127 < e && 2048 > e ? a += String.fromCharCode(e >> 6 | 192) : (a += String.fromCharCode(e >> 12 | 224),
                    a += String.fromCharCode(e >> 6 & 63 | 128)),
                    a += String.fromCharCode(e & 63 | 128))
                }
                f = a;
                d = [];
                for (e = 0; e < 8 * f.length; e += 8)
                ...
                d = "";
                for (f = 0; f < 4 * a.length; f++)
                    d += "0123456789abcdef".charAt(a[f >> 2] >> 8 * (3 - f % 4) + 4 & 15) + "0123456789abcdef".charAt(a[f >> 2] >> 8 * (3 - f % 4) & 15);
                return d
            }
            a.jsonp && (ub = 0);
              ...
              , I = {
                ...
                history: function(a, b) {
                    var b = a.callback || b
                      ...
                      , u = L();
                    if (!m)
                        return s("Missing Channel");
                    if (!b)
                        return s("Missing Callback");
                    if (!w)
                        return s("Missing Subscribe Key");
                    t.stringtoken = "true";
                    t.count = c;
                    t.reverse = d;
                    t.auth = f;
                ...
                replay: function(a, b) {
                    var b = b || a.callback || C()
                      ...
                      , r = {};
                    if (!d)
                        return s("Missing Source Channel");
                    if (!e)
                        return s("Missing Destination Channel");
                    if (!P)
                        return s("Missing Publish Key");
                    if (!w)
                        return s("Missing Subscribe Key");
                    "0" != p && (r.callback = p);
                    f && (r.stop = "all");
                    ...
                    function f() {
                        ...
                        if (b) {
                            ...
                            Ha = E({
                                ...
                                c: function(a) {
                                    if (!a || "object" == typeof a && "error"in a && a.error)
                                        return x(a.error),
                                        setTimeout(d, D);
                                    J(a[1]);
